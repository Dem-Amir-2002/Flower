...
if(isset($_POST['btn_save'])) {
    ...
    $now = time();
    $data = $conn->prepare("INSERT INTO `articles` (`catid`,`title`,`short`,`content`,`pic`,`created_at`) VALUES (?,?,?,?,?,?)");
    $data->bindParam(1,$cat);
    $data->bindParam(2,$title);
    $data->bindParam(3,$short);
    $data->bindParam(4,$content);
    $data->bindParam(5,$pic);
    $data->bindParam(6,$now);
    if($data->execute()){
        $pic = '';
        if(!empty($_FILES['pic']['name'])){
...
if(isset($_POST['btn_update'])){
    ...
    $data = $conn->prepare("UPDATE `articles` SET `catid`=?,`title`= ?,`short`=?,`content`=?,`pic`=? WHERE `id`=?");
    $data->bindParam(1,$cat);
    $data->bindParam(2,$title);
    $data->bindParam(3,$short);
    $data->bindParam(4,$content);
        ...
        <div class="col-10 admin-content text-right py-3">
            ...
            <form action="" method="POST" enctype="multipart/form-data">
                <div class="form-group row">
                    ...
                    <div class="col-12">
                        <label>محتوا</label>
                        <textarea rows="5" class="form-control" name="short"><?=@$erows['short']?></textarea>
                    </div>
                    <div class="col-12">
                        <label>محتوا</label>
