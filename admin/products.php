...
$data = $conn->query("SELECT * FROM `products` WHERE TRUE $where ORDER BY `created_at` DESC");
$total = $data->rowCount();
//delete
            ...
            <table class="table table-striped table-hover text-center">
                ...
                <?php if($total){ $count = 1; while($rows = $data->fetch()){ ?>
                        <tr>
                            ...
                            <td>
                                <a href="edit.php?id=<?=$rows['pid']?>"class="btn btn-xs btn-primary">ویرایش</a>
                                <a href="products.php?delete=<?=$rows['pid']?>" onclick="return confirm('آیا از حذف اطمیان دارید؟ این عمل غیر قابل بازگشت می باشد.')" class="btn btn-xs btn-danger">حذف</a>
                            </td>
                        </tr>
                <?php } }else{ ?>
