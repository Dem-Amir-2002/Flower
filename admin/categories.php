...
//categoru
$cat = $conn->query("SELECT *,(SELECT COUNT(`catid`) FROM `products` WHERE `catid`=`cid`) as `count` FROM `categories` ORDER BY `cname` ASC");
//submit data
if(isset($_POST['btn_save'])) {
    ...
    $data = $conn->prepare("INSERT INTO `categories` (`cname`,`pic`) VALUES (?,?)");
...
if(isset($_POST['btn_update'])){
    ...
    $data = $conn->prepare("UPDATE `categories` SET `cname`= ?,`pic`=? WHERE `cid`=?");
    $data->bindParam(1,$title);
    $data->bindParam(2,$pic);
    $data->bindParam(3,$id);
    if($data->execute()){
        ...
        <div class="col-10 admin-content text-right py-3">
            ...
            <form action="" method="POST" enctype="multipart/form-data">
                <div class="form-group row">
                    <div class="col-12">
                        <label>عنوان</label>
                        <input type="text" class="form-control" name="title" placeholder="عنوان گروه" value="<?=@$erows['cname']?>">
                    </div>
                ...
                <table class="table table-striped table-hover text-center">
                    ...
                    <?php $count=1; while($rows = $cat->fetch()){ ?>
                        <tr>
                            ...
                            <td><?=$rows['count']?></td>
                            <td>
                                <a href="categories.php?edit=<?=$rows['cid']?>" class="btn btn-xs btn-primary">ویرایش</a>
                                <a href="categories.php?delete=<?=$rows['cid']?>" onclick="return confirm('آیا از حذف اطمیان دارید؟ این عمل غیر قابل بازگشت می باشد.')" class="btn btn-xs btn-danger">حذف</a>
                            </td>
                        </tr>
                    <?php } ?>
                    </tbody>
