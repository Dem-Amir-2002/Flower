...
//get new order
$new = $conn->query("SELECT * ,`orders`.`count` as `ocount`,`orders`.`status` as `ordst` FROM `orders` 
    INNER JOIN `products` ON `orders`.`productid`=`products`.`pid` 
   WHERE `orders`.`userid` = $userid AND `orders`.`status` = 0 ORDER BY `order_date` DESC");
    ...
    <div class="container">
        <div class="d-flex justify-content-start align-items-center px-3 py-5">
            ...
            <div class="d-flex align-items-center ms-3">
                ...
                <h6 class="text-muted mb-0"> سفارش های من </h6>
            </div>
        </div>
        <div class="row pb-5">
            <div class="col-md-4">
