...
if(isset($_POST['submit'])){
    ...
    $reply_id = isset($_POST['reply_id']) && $_POST['reply_id'] !== '' ? $_POST['reply_id'] : NULL;

    $stmt = $conn->prepare("INSERT INTO `comments` (`userid`, `proid`, `comment`, `comment_date`, `reply_id`) 
                            VALUES (:userid, :proid, :comment, :cdate, :reply_id)");
    $stmt->bindValue(':userid', $userid, PDO::PARAM_INT);
    $stmt->bindValue(':proid', $id, PDO::PARAM_INT);
    $stmt->bindValue(':comment', $comment, PDO::PARAM_STR);
    $stmt->bindValue(':cdate', time(), PDO::PARAM_INT);
    $stmt->bindValue(':reply_id', $reply_id, is_null($reply_id) ? PDO::PARAM_NULL : PDO::PARAM_INT);
    $stmt->execute();
}


//ثبت نظر
function showComments($conn, $proid, $reply_id = NULL, $level = 0)
{
    $sql = "SELECT c.*, u.name, u.family, u.permission 
            FROM comments c 
            INNER JOIN users u ON c.userid = u.ID 
            WHERE c.proid = :proid AND " .
        ($reply_id === NULL ? "c.reply_id IS NULL" : "c.reply_id = :reply_id") .
...
?>
        <div class="alert alert-success" style="margin-right:<?= $level * 40 ?>px;">
            <h6>
                <?= $row['permission'] == 2 ? '<i class="fa fa-star text-warning"></i>' : '' ?>
                <?= $row['name'] . ' ' . $row['family'] ?>
            </h6>
            <p><?= $row['comment'] ?></p>
            <span class="float-left"><i class="fa fa-clock"></i> <?= jdate('Y/m/d', $row['comment_date']) ?></span>
        </div>
<?php
        // بازگشتی برای نمایش پاسخ‌ها
        showComments($conn, $proid, $row['comment_id'], $level + 1);
    }
}
                ...
                <div id="menu1" class="tab-pane active px-5 py-3">
                    ...
                    <?php if (isset($_SESSION['userid'])) { ?>
                        ...
                        <form action="" method="post">
                            <input type="hidden" name="reply_id" value="<?= isset($_GET['reply_id']) ? intval($_GET['reply_id']) : '' ?>">
                            <div class="form-group">
                                <textarea class="form-control bg-light" name="comment" cols="100" rows="7"></textarea>
                            </div>
                            <div class="form-group mt-2">
                                <button type="submit" name="submit" class="btn btn-success">
                                    <?= isset($_GET['reply_id']) ? 'ثبت پاسخ' : 'ثبت نظر' ?>
                                </button>
                            </div>
                        </form>
                    <?php } else { ?>
